...
const Navbar: React.FC<NavbarProps> = ({
  ...
  onEnterReview, onExitReview, onNavigateAdmin
}) => (
  <header className="
...
function App() {
  ...
  const loadSpecificArticleInMode = async (
    ...
    try {
      ...
      let firstIndex = 0;
      if (targetTripleId) {
        // Jump to exact triple by ID - most reliable
  ...
  const enterReviewMode = async (reviewMode: 'review-skipped' | 'review-flagged') => {
    ...
    try {
      ...
      setReviewTripleIds(tripleIds);  // New state - add to useState
      setReviewIndex(0);
      // setMode(reviewMode);
      // setReviewPmids(pmids);
      // setReviewIndex(0);
    ...
    return (
      <div className="h-screen flex flex-col overflow-hidden bg-slate-50 dark:bg-slate-900">
        <AdminNavbar
          ...
          onBack={() => navigate('/')}
        />
        {/* Admin body scrolls, header stays fixed */}
